...
@Singleton
class SnoozeRepositoryImpl @Inject constructor(
    ...
    override suspend fun clearIfFor(alarmId: Long): Boolean {
        if (pendingAlarmId.first() != alarmId) return false
        clear()
        return true
    }

    private object Keys {
        val PENDING_ALARM_ID = longPreferencesKey("snooze_alarm_id")
        val PENDING_AT = longPreferencesKey("snooze_at")
    }
}
